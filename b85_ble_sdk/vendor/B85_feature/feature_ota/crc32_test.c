...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//码表生成时间统计
//注意：码表的存储有3种方式：1.预先算好的table直接存储在flash中; 2.预先算好的table存储在全局变量数组中 ; 3.分配local变量，堆栈中，每次重新计算
//只有上面第3种方式才需要重新生成码表。

	//计算crc32 table[256*4]（对应byte单位数据的crc计算）。耗时 1.6 ms
	//计算出来的表AA_crc_table和预先定义在flash上的crc32_tbl肯定是一样的（因为crc32_tbl也使用poly值为0x04C11DB7计算的）
    DBG_CHN0_HIGH;
    crc32_init(0x04C11DB7, AA_crc_table);  //1600us
    DBG_CHN0_LOW;


    //计算crc32 half table[16*4]（对应半个byte单位数据的crc计算）.耗时150us
	//计算出来的表AA_crc_half_table和预先定义在flash上的crc32_half_tbl肯定是一样的（因为crc32_half_tbl也使用poly值为0x04C11DB7计算的）
    DBG_CHN1_HIGH;
...
////////////////////////下面用四种方法对同一笔数据进行crc32的计算，计算结果一样，只是sram/flash/timing上各有差异
    //crc初值全部使用 0xFFFFFFFF
...
#if 0
    //1.按byte计算crc32，查sram上的表AA_crc_table（占用1K byte sram）AA_crc_table，耗时64us
    DBG_CHN0_HIGH;
    AAA_crc_res1 = crc32_cal(AAA_crc_res1, test16, AA_crc_table, 16);  //计算结果：99e17457
    DBG_CHN0_LOW;

    //2.按byte计算crc32，查flash上的表crc32_tbl（占用1K byte Flash），耗时212us
    DBG_CHN1_HIGH;
    AAA_crc_res2 = crc32_cal(AAA_crc_res2, test16, crc32_tbl, 16);  //计算结果：99e17457
    DBG_CHN1_LOW;
#endif

    //3.按半个byte计算crc32，查sram上的表crc32_tbl（占用64 byte sram）AA_crc_table，耗时100us
    DBG_CHN2_HIGH;
    AAA_crc_half_res1 = crc32_half_cal(AAA_crc_half_res1, test32, AA_crc_half_table, 32);  //计算结果：99e17457
    DBG_CHN2_LOW;

    //4.按半个byte计算crc32，查flash上的表crc32_half_tbl（占用64 byte Flash），耗时136us
    DBG_CHN3_HIGH;
    AAA_crc_half_res2 = crc32_half_cal(AAA_crc_half_res2, test32, crc32_half_tbl, 32);    //计算结果：99e17457
    DBG_CHN3_LOW;
...
////////////////////////下面用四种方法对同一笔数据进行crc32的计算，计算结果一样，只是sram/flash/timing上各有差异
	...
	//1.按byte计算crc32，查sram上的表AA_crc_table（占用1K byte sram）AA_crc_table，耗时64us
	DBG_CHN0_HIGH;
	AAA_crc_res3 = crc32_cal(AAA_crc_res3, test16_Another, AA_crc_table, 16);  //计算结果：d0c7d4de
	DBG_CHN0_LOW;

	//2.按byte计算crc32，查flash上的表crc32_tbl（占用1K byte Flash），耗时212us
	DBG_CHN1_HIGH;
	AAA_crc_res4 = crc32_cal(AAA_crc_res4, test16_Another, crc32_tbl, 16);  //计算结果：d0c7d4de
	DBG_CHN1_LOW;

	//3.按半个byte计算crc32，查sram上的表crc32_tbl（占用64 byte sram）AA_crc_table，耗时80us
	DBG_CHN2_HIGH;
	AAA_crc_half_res3 = crc32_half_cal(AAA_crc_half_res3, test32_Another, AA_crc_half_table, 32);  //计算结果：d0c7d4de
	DBG_CHN2_LOW;

	//4.按半个byte计算crc32，查flash上的表crc32_half_tbl（占用64 byte Flash），耗时136us
	DBG_CHN3_HIGH;
	AAA_crc_half_res4 = crc32_half_cal(AAA_crc_half_res4, test32_Another, crc32_half_tbl, 32);    //计算结果：d0c7d4de
	DBG_CHN3_LOW;
    ...
    AAA_crc_res5 ^= 0xFFFFFFFF;  //计算结果：261daee5，注意这个异或不是必须的
    write_reg32(0x40004, AAA_crc_res5);
    DBG_CHN0_LOW;



    //将16 byte的数据流test16_Another拆成2笔 8byte的数据流，用前8个计算的结果作为后8个的crc_init，验证结果是否一致
    //计算结果 d0c7d4de 与前面一致
    u32 temp0 = crc32_cal(0xFFFFFFFF, test16_Another, AA_crc_table, 8);
    u32 temp1 = crc32_cal(temp0, test16_Another + 8, AA_crc_table, 8);
    write_reg32(0x40004, temp1);  //8255 0x40004，若其他IC 0x8004


    write_reg8(0x40000, 0x33);  //8255 0x40000，若其他IC 0x8000
    if(AAA_crc_res5){
        while(1);
    }
    write_reg8(0x40000, 0x44);   //8255 0x40000，若其他IC 0x8000
#endif
}
